...
| [`Limpar Memoria` fora do versionamento](PENDENCIA-LIMPAR-MEMORIA.md) | webhook ATIVO na pasta do agente, 8 nos, terminando em `DEL` de Redis, sem nenhuma copia em `n8n/workflows/` e sem nenhum chamador em `src/` — so duas mencoes em doc. Ou versiona, ou desativa, ou escreve por que existe | **agora**: as tres saidas dependem de saber quem chama, e isso nao fica mais facil com o tempo |
