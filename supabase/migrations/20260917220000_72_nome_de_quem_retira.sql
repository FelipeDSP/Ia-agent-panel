-- =====================================================================
-- 72 — Quem vai retirar o pedido
-- =====================================================================
-- Pedido do Felipe, 17/09: o aviso ao dono levava só o nome do perfil do
-- WhatsApp (apelido, ou vazio) e quem retira nem sempre e quem conversou.
--
--   * `pedidos.retirada_nome` — coluna propria (o painel mostra; o aviso leva).
--   * `api_n8n_fechar_pedido` (mesma assinatura) le `nome_retirada` (ou
--     `nome`) do jsonb que ja recebia e, quando a conta pediu
--     (`tenant_tools.config.pedir_nome = true`), RECUSA fechar sem ele —
--     "NADA FOI FECHADO: falta o nome de quem vai retirar…". Ausente = nao
--     pede: o n8n congelado nunca passa nome e nao pode ser barrado.
--   * `api_n8n_notificar_venda` / `api_agente_aviso_pedido` (mesmas
--     assinaturas) escrevem "🙋 Retira: <nome>".
--
-- O ENDERECO de retirada nao e daqui: e config do painel
-- (`vendas.config.retirada.{endereco,mapa_url}`) que o SERVICO manda ao
-- cliente em mensagem propria depois do fechamento.
--
...
alter table public.pedidos add column if not exists retirada_nome text;
comment on column public.pedidos.retirada_nome is
  '72: nome de quem vai retirar, informado ao agente ao fechar (config.pedir_nome torna obrigatorio).';

-- ---- api_n8n_fechar_pedido(uuid,bigint,text): corpo da 70/69 com o nome de quem retira (72) ----
...
begin
  ...
  -- 72: quem vai retirar. `nome_retirada` (a tool) ou `nome` (o modelo solto);
  -- a conta decide se e obrigatorio (`config.pedir_nome`, ausente = nao —
  -- o n8n congelado nunca passa nome e nao pode ser barrado por isto).
  v_nome := left(btrim(coalesce(nullif(btrim(coalesce(v_meta ->> 'nome_retirada', '')), ''), v_meta ->> 'nome', '')), 120);
  select coalesce((tt.config ->> 'pedir_nome')::boolean, false) into v_pedir_nome
    from public.tenant_tools tt where tt.tenant_id = p_tenant_id and tt.tool_nome = 'vendas';
  if coalesce(v_pedir_nome, false) and v_nome = '' then
    return 'NADA FOI FECHADO: falta o nome de quem vai retirar. Pergunte ao cliente em nome de quem '
        || 'fica o pedido e chame fechar de novo com nome_retirada preenchido.';
  end if;
  ...
  set status         = 'aguardando_pagamento',
      ...
      retirada_nome  = nullif(v_nome, ''),
      -- as chaves viraram coluna; o resto (observacao, entrega antiga…) fica
      metadados      = p.metadados || (v_meta - 'pagamento' - 'modalidade' - 'nome_retirada' - 'nome')
  where p.id = v_pedido;

  if v_pagamento = 'na_retirada' then
    return format(E'Pedido nº %s fechado — retirada no local, pagamento NA RETIRADA (nao gere link; '
                  || E'nao diga que esta pago).%s\n%s', v_numero,
                  case when v_nome <> '' then E'\nRetira: ' || v_nome else '' end,
                  public.pedido_em_texto(v_pedido));
  end if;
  return format(E'Pedido nº %s fechado.%s\n%s', v_numero,
                case when v_nome <> '' then E'\nRetira: ' || v_nome else '' end,
                public.pedido_em_texto(v_pedido));
end;
$function$
...
-- ---- api_n8n_notificar_venda(uuid,bigint,integer): corpo da 70/69 com o nome de quem retira (72) ----
...
begin
  ...
  select p.id, p.numero, p.total_centavos, p.metadados, p.pagamento_modo, p.modalidade, p.retirada_nome
    into v_pedido, v_numero, v_total, v_meta, v_modo, v_modal, v_retira
  ...
  v_msg :=
    ...
    -- 72: quem vai buscar, quando o cliente informou
    || case when coalesce(btrim(v_retira), '') <> '' then format(E'\n🙋 Retira: %s', btrim(v_retira)) else '' end
...
-- ---- api_agente_aviso_pedido(uuid,bigint,text,uuid,integer): corpo da 70/69 com o nome de quem retira (72) ----
...
begin
  ...
  select p.id, p.numero, p.total_centavos, p.pagamento_modo, p.modalidade, p.retirada_nome
    into v_pedido, v_numero, v_total, v_modo, v_modal, v_retira
  from public.pedidos p
  where p.tenant_id       = p_tenant_id
    and p.conversation_id = p_conversation_id
