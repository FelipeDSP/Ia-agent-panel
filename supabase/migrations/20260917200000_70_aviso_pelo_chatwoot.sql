-- =====================================================================
-- 70 — Aviso ao dono pela inbox do agente (canal `chatwoot`, sem sessão)
-- =====================================================================
-- Decisão do Felipe, 17/09: nenhuma conta precisa de sessão WAHA para ser
-- avisada — o aviso sai pelo Chatwoot, pela inbox do próprio agente, com o
-- token de usuário da agência (`CHATWOOT_AGENCIA_TOKEN`, uma credencial só).
--
-- Aqui só o banco: `api_n8n_notificar_venda` e `api_agente_aviso_pedido`
-- (mesmas assinaturas) passam a devolver a linha com DESTINO quando o canal é
-- `chatwoot` OU `waha`, com `sessao` nula quando não há sessão — o serviço
-- escolhe o caminho (sessão → WAHA; sem sessão → Chatwoot). Antes, sem sessão
-- a linha não saía. O n8n congelado (Empório) continua chamando
-- `notificar_venda` e testando "tem destino"; o Empório não tem config de
-- vendas, então nada muda para ele.
--
-- `create or replace` de mesma assinatura: ACL preservado (o teste confere
-- por diff). Extensão: nenhuma. REEXECUTÁVEL.
-- =====================================================================

begin;

-- ---- api_n8n_notificar_venda(uuid,bigint,integer): corpo da 69 com a condicao do canal aberta (70) ----
...
-- ---- api_agente_aviso_pedido(uuid,bigint,text,uuid,integer): corpo da 69 com a condicao do canal aberta (70) ----
