/**
 * Migração 76 — o consumo conta de 21/09/2026.
 *
 ...
 *
 *   npm run teste:migracao-consumo-zera
 */
...
const M76 = leia('20260921200000_76_consumo_zera_antes_de_21_09.sql');
const semTx = (s) => s.replace(/^\s*(begin|commit)\s*;\s*$/gim, '');
const CORTE = '2026-09-21T00:00:00Z';
...
try {
  ...
  chk('B de hoje: intocada (5000/100, cache 400, openai_usage)', h.te === 5000 && h.ts === 100 && h.tc === 400 && h.f === 'openai_usage', JSON.stringify(h));
  ...
  chk('soma de hoje de B = 5100 (a 76 não tocou no dia corrente)', hoje.s === 5100, String(hoje.s));
} finally {
